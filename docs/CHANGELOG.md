...
GPIO 2. On battery it draws less than the blank, which is 3.28 W measured. The first 20-minute
comparison puts the sleep somewhere between ~0.6 and ~2.7 W. The fuel gauge's 1 % steps and
the battery's recovery during a light load leave that range wide, and a longer run is needed
to narrow it. That number decides whether s2idle is worth having. The last is the one that decides whether s2idle is worth having: Pi 5 has no cpuidle
